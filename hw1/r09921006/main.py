...
from face_sdk.api_usage.crop_apd_by_sdk import crop_apd
...
if __name__ == '__main__':
    ...
    conf.add_argument('--model_path', type = str, default = './trained_model/Epoch_184.pt', 
                      help = 'The path of model or the directory which some models in.')

    ### use default value ###
    conf.add_argument("--data_root", type = str, default='./data_train/C_prep', 
                      help = "The root folder of training set.")
    conf.add_argument("--train_file", type = str, default='./data_files/train/train_list_unsupervised.txt', 
    ...
    if args.mode == 'train':
        prep_train_data('./data/train/C', './data_train/C_prep')
        prep_train_data('./data/test/open_set/unlabeled_data', './data_test/open_set/unlabeled_data_prep')
        gen_train_list('./data_train/C_prep', './data_files/train/train_list_unsupervised.txt', './data_test/open_set/unlabeled_data_prep')
        train_main(args)
    if args.mode == 'closed':
        crop_apd('./data/test/closed_set/test_pairs', './data_test/closed_set/test_pairs_crop')
        gen_image_list('./data_test/closed_set/test_pairs_crop', './data_files/test/closed_set/img_list_closed.txt')
        gen_pairs_file('./data_test/closed_set/test_pairs_crop', './data/test/closed_set/labels.txt', './data_files/test/closed_set/pairs_closed.txt')
        args.test_set = 'APD'
        test_main(args)
    if args.mode == 'open':
        crop_apd('./data/test/open_set/test_pairs', './data_test/open_set/test_pairs_crop')
        gen_image_list('./data_test/open_set/test_pairs_crop', './data_files/test/open_set/img_list_open.txt')
        gen_pairs_file('./data_test/open_set/test_pairs_crop', './data/test/open_set/labels.txt', './data_files/test/open_set/pairs_open.txt')
        args.test_set = 'OPEN'
        test_main(args)
